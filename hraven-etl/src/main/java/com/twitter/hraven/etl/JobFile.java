...
public class JobFile implements Writable {
    ...
    private static final String SPARK_LOG_NAME = "EVENT_LOG_1";
    ...
    private boolean isSparkFile = false;
     ...
     * Constructor
     *
     * @param name of the file (that may or may not be a job file).
     */
    ...
    /**
     * create JobFile object for Spark history file
     * */
    public JobFile(Path p) {
        parseFilename(p);
    }

    /**
     * Unlile parse MR job from file name,
     * parse spark job id form path
     * */
    private void parseFilename(Path p){
        filename = p.getName();
        jobid = p.getParent().getName().replaceFirst("^application","job");
        if(p.getParent().getName().startsWith("application") &&
                filename.equals(SPARK_LOG_NAME)) {
            isSparkFile = true;
            LOG.debug("Spark file in dir" + p.getParent().getName() + " with job id: " + jobid);
        }
    }
    ...
    /**
     * @return whether this file is a Spark History File
     */
    public boolean isSparkFile() {
        return isSparkFile;
    }
