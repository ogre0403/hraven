...
public class MinMaxJobFileTracker {
    ...
    private static Log LOG = LogFactory.getLog(MinMaxJobFileTracker.class);
    /**
     ...
     *
     * @param jobFileStatus of a jobfile, must be a proper JobFile. Cannot be null.
    ...
    public JobFile track(FileStatus jobFileStatus) {
        ...
        if (jobFile.isJobConfFile() || jobFile.isJobHistoryFile()) {
            ...
            LOG.info("JobFile for MR job");
        }
        return jobFile;

    }

    public JobFile trackSpark(FileStatus jobFileStatus) {
        JobFile sparkFile = new JobFile(jobFileStatus.getPath());
        if(sparkFile.isSparkFile()){
            track(sparkFile.getJobid());
            long modificationTimeMillis = jobFileStatus.getModificationTime();
            if (modificationTimeMillis < minModificationTimeMillis) {
                minModificationTimeMillis = modificationTimeMillis;
            }
            ...
            LOG.info("JobFile for Spark Job");
        }
        return sparkFile;
    }
    ...
    /**
     * @param jobIdString to be tracked.
     */
